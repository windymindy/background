...
void test_service::setting_with_running_as_console_application_disables_error_1 ()
{
    ...
    QVERIFY (! service.configuration ().has_value ());
...
void test_service::ignoring_not_system_service_error_runs_as_console_application_1 ()
{
    ...
    QCOMPARE (service.running_as_service ().value (), false);
    QVERIFY (! service.configuration ().has_value ());
    QVERIFY (! service.error ().has_value ());
    QVERIFY (! failed.isEmpty ());
...
unsigned int service_platform_plugin_test::order ()
{
    return 1;
}
...
bool service_platform_test::check ()
{
    ...
    if (! signal_utility::connected (this, checked))
        return true;
    return result;
}
...
{
    Q_EMIT start_ ();
    if (! signal_utility::connected (this, started_))
        Q_EMIT started ();
}
...
{
    Q_EMIT stop_ ();
    if (! signal_utility::connected (this, stopped_))
        Q_EMIT stopped ();
}
...
void service_platform_test::set_state_serving ()
{
    ...
    if (! signal_utility::connected (this, state_serving_set_))
...
void service_platform_test::set_state_stopping ()
{
    ...
    if (! signal_utility::connected (this, state_stopping_set_))
...
void service_platform_test::set_state_stopped ()
{
    ...
    if (! signal_utility::connected (this, state_stopped_set_))
...
void service_platform_test::retrieve_configuration ()
{
    ...
    if (! signal_utility::connected (this, configuration_retrieved_))
...
unsigned int console_platform_plugin_test::order ()
{
    return 1;
}
